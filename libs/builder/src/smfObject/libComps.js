...
module.exports = (function() {
  ...
  function prepareLibComps(libraryPage) {
    var res = {};
    const smfObjects = (libraryPage.smfObjects || []).slice();
    // find library components.
    if (libraryPage.footer.bottomTabbar) {
      smfObjects.push(libraryPage.footer.bottomTabbar);
    }
    smfObjects.forEach(comp => {
      if (comp.parent === LIBRARY_PAGE_NAME) {
        if (comp.type === 'ScrollView') comp.isNeedScwLayoutProps = true;
        res[comp.id] = comp;
    ...
    findAndSetLibComponents(smfObjects, res);
  ...
  function getLibraryNestedChildrenTestIDs(id, varName) {
    ...
    if (libraryPageComps && libraryPageComps[id] && libraryPageComps[id].children) {
      const childRes = libraryPageComps[id].children.map(
        item => `${varName}_${util.capitalizeFirstLetter(item.name)}`
      );
      ...
      libraryPageComps[id].libID &&
      libraryPageComps[id].libID !== libraryPageComps[id].id
    ) {
    ...
    smfObjects.forEach(comp => {
      if (comp.parent !== LIBRARY_PAGE_NAME) {
        ...
        if (parentLibComp && !checkLibComponentAlreadyExist(comp, comps)) {
          if (comp.type === 'ScrollView') parentLibComp.isNeedScwLayoutProps = true;
        }
      }
