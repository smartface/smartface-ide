...
export type FilesDetailType = {
    [key: string]: { scaleFactor: number; scale: number; crc?: number; date?: Date; path?: string; fullPath?: string }
}
...
export default class Workspace {
     ...
     */
    async getImage(device: Device, imageName?: string | string[], full?: boolean): Promise<FilesDetailType> {
        if (!(device instanceof Device)) device = new Device(device);
        full = !!full;
