...
function getRequiredIrregularEnums(smfObjects) {
  ...
  for (var mdl in mdls) {
        if(IRREGULAR_ENUMS.require[mdl])
            ...
            res.push(`import ${mdl} from '${IRREGULAR_ENUMS.import[mdl]}'`);
  }
  return res.join("\n");
}
...
module.exports = function(_compiler) {
  return {
    ...
    removeFrom: (smfObjects, removes) => {
        return removes && removes.length ? smfObjects.filter(item => !removes.some(remove => item.type === remove)) : smfObjects;
    }
  };
};
