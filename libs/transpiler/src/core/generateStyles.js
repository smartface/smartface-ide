...
function createReplacerRegExp(variables) {
    const keyVars = Object.keys(variables).map((key) => escapeRegExp(`"\${${key}}"`)).join("|");
                ...
                generateOneTheme(defaultThemePath) // generate defaultBundle
                    ...
                    .then(workspaceThemeDist => {
                        files.length ? files.forEach((file, index) => {
                            isExistsFileDir(path.join(themesFolder, file))
                                .then(res => {
                                    if (res.dir) { // generate others
                                        promises.push(generateOneTheme(path.join(themesFolder, file), workspaceTheme));
                                    }
                                    done();
                                }, reject);
                        }) : done();
    ...
    function generateOneTheme(themeDir, defaultParentTheme) {
        return new Promise((resolve, reject) => {
            ...
            recursiveReaddir(path.join(themeDir, "styles")).then(files => {
                ...
                getThemePack(path.dirname(themeDir), parentTheme, defaultParentTheme)
                    .then(parentThemePack => {
                        let variablesJson = {};
    ...
    function getThemePack(themesFolder, theme, defaultParentTheme, _count) {
        return new Promise((resolve, reject) => {
            ...
            if (themeBundles[theme]) {
                ...
                setTimeout(e => {
                    getThemePack(themesFolder, theme, defaultParentTheme, _count ? (_count + 1) : 2)
                        .then(resolve, reject);
                }, 200);
            }
            else {
                generateOneTheme(path.join(themesFolder, theme), defaultParentTheme)
                    .then(resolve, reject);
            }
        });
    }
...
function eliminateAttributesFromStyle(styleObj, isDefault) {
    ...
    if (styleObj && (typeof styleObj === "object")) {
        ...
        if (keys.length) {
            keys.forEach(key => {
                if (isDefault && IGNORE_DEFAULTS[key] !== undefined && IGNORE_DEFAULTS[key] === styleObj[key]) {
                    delete styleObj[key];
                    ++deleted;
                    return;
                }
